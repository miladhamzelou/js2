...
CmpByJSON=['20270','26260'];//CmpByJSON.indexOf(CmpId)>-1
   ...
   $.widget( "np.npform", {
      ...
      _create:function(){
         ...
         if (!op.IsCmp&&!op.UseOpenerAut){
            ...
            if (CmpByJSON.indexOf(op.CmpId)>-1){
      ...
      _Init_SndXML:function(myParam){
         var self = this,
             ...
             //CmpId=myParam?(myParam.CmpId||'00'):'00';
             IsJSON=op.CmpId&&(CmpByJSON.indexOf(op.CmpId)>-1);
         if ((Type)&&(Type=='nav')){
      ...
      ResetClick:function(){//شروع مجدد
         var self = this;
         self._ShowErrorBox(self.EmptyMessage);
         self.Reset();
         self.InitFrm();
         return;
      }, // end of ResetClick()
      ...
      DoAct:function(act,HelpId,/*ishelp,*/Params){
         ...
         */   
         ...
         var IsJSON=(op.CmpId&&(CmpByJSON.indexOf(op.CmpId)>-1))?true:false,
             ajaxdata;
         if (IsJSON){
            //var xml=$($.parseXML('<?xml version="1.0" encoding="utf-8"?><soap:Envelope xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xsd="http://www.w3.org/2001/XMLSchema" xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/"><soap:Body><srv xmlns="urn:nowpardaz"><root><x><r /></x></root></srv>  </soap:Body></soap:Envelope>'))
            //$('r',xml).attr('xx',JSON.stringify(self._SndXML));
            //ajaxdata=xml.xml2();
         ...
         $.ajax({
            ...
            beforeSend: function(xhr) {
               ...
               (!IsJSON)&&
               xhr.setRequestHeader("SOAPAction", "urn:nowpardaz/NPService/srv"); 
            },
            complete: function(xhr, msg){
               self.ParallelActCnt--;
               self.WebBusy=false;
      ...
      Get_Data:function(HelpId, Params) {
         ...
         self._ShowErrorBox(self.Resp,true);
         if (Params && Params.AutoComplete) {
      ...
      _ShowErrorBox:function(data, AppendToResponse){
         var self=this,e = self.element, op=self.options;
         if (op.ParentCmp){
            self=op.ParentCmp;
            ...
            return self._ShowErrorBox(data, AppendToResponse);
         }
         var $data=data,//$(data),
             $Msgs=$("msg", $data);
         if ((!AppendToResponse)||(!self.W_.$Msgs)){
            self.W_.$Msgs=$("msg", $data).clone();
         }
         else{
            self.W_.$Msgs
         ...
         //$('input:eq(0)',self.W_.$MessageHeaderDialog).prop('checked',$Error.length?true:false);   
         //$('input:eq(1)',self.W_.$MessageHeaderDialog).prop('checked',$Warning.length?true:false);   
         //$('input:eq(2)',self.W_.$MessageHeaderDialog).prop('checked',$Suc.length?true:false);
         //$('input:eq(0)',self.W_.$MessageHeaderDialog).button('refresh');   
         //$('input:eq(1)',self.W_.$MessageHeaderDialog).button('refresh');   
         //$('input:eq(2)',self.W_.$MessageHeaderDialog).button('refresh');   
         //$('.ui-button:eq(0)',self.W_.$MessageHeaderDialog)
         //   .find('.ui-button-text').text($Error.length+' '+$ErrorMessageText);
         //$('.ui-button:eq(1)',self.W_.$MessageHeaderDialog)
         //   .find('.ui-button-text').text($Warning.length+' '+$WarningMessageText);
         //$('.ui-button:eq(2)',self.W_.$MessageHeaderDialog)
         //   .find('.ui-button-text').text($Suc.length+' '+$SucMessageText);
      ...
      _XML2ViewModel:function(data,Params){//SetInParam, DontStopSubscribe){
         var self = this,op=self.options, e = self.element;
         var vformated;
         var IsJSON=false;//op.CmpId&&(CmpByJSON.indexOf(op.CmpId)>-1);
         ...
         if(IsJSON){ 
            $.each(data.rset.grd,function(Name,Value){
               var $v=v,
                   id=Value.id;
               ...
               if ((v.Type=='Cmp')&&(!v.md)&&(!v.IsFaci)){//if ((self.W_.FrmFld[id].Type=='Cmp')&&(!self.W_.FrmFld[id].md)&&(!self.W_.FrmFld[id].IsFaci)){
                  if (!v.Valid.RO||(v.Valid.RO!=="1")){//if (!$v.attr('RO')||($v.attr('RO')!=="1")){
                     ...
                     if (vfix!==''){
                        ...
                        if (!_id) _id=id;
                        if (IsJSON)
                           a.push({name:_id,val:vfix});//a[_id]=vfix;
                        else
                           a.attr(_id,vfix);
      ...
      RunAjax:function(SndXml,Complete,Error){
         var self=this;
      ...
      OpenNewForm:function(ftype,fid,subfrm,BrnNo,BrnLimit,Params){//AfterOpen,NewFormReturnCallback){
         ...
         self.url="/frm/nav/nav.svc";
         self.RunAjax(self._SndNavXML.xml2(),
            function(xhr, msg){
               if(msg == 'success'){
                  var NavResp=$.parseXML(xhr.responseText),
                      NavOutPar=$('oaut',NavResp),
                      FatherTicket=NavOutPar.attr('nmtck');
                  if (FatherTicket){
                     self.SetTckByResp(FatherTicket);
                  }
                  if ($('msg>error',NavResp).length){
                     self._ShowErrorBox(NavResp,Params.ParentCmp);
                     self.CloseWaitingDialog();
                  }
                  else{
                     if (!Params.ParentCmp)
                        self._ShowErrorBox(self.EmptyMessage);                     
                     Params.AddrPath=NavOutPar.attr('nurlp')+'/',
                     Params.FName=NavOutPar.attr('nurlf'),
                     Params.FTitle=NavOutPar.attr('nam'),
                     Params.NewFormAut=$('aut',NavResp);//.clone();
                     if ((Params.AddrPath=='/')&&(!Params.FName)){
                        ...
                        self._ShowErrorBox(xx,Params.ParentCmp);
                        self.CloseWaitingDialog();
                     }
                     else{
                        self.GetNewForm(Params);//, CallBack);
